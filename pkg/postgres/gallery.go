...
func (qb *GalleryStore) setGallerySort(query *queryBuilder, findFilter *models.FindFilterType) error {
	...
	case "performer_age":
		...
		var fallback string
		if direction == "ASC" {
			// ASC puts NULL first by default, so coalesce to max int.
			fallback = "9223372036854775807"
		} else {
			// DESC puts larger values first; coalesce NULL to min int to keep NULLs last.
			fallback = "-9223372036854775808"
		}
		query.sort += fmt.Sprintf(
			" ORDER BY (SELECT COALESCE(%s(galleries.date - performers.birthdate), %s) FROM %s as performers INNER JOIN %s AS aggregation ON performers.id = aggregation.%s WHERE aggregation.%s = %s.id) %s",
			aggregation,
			fallback,
			performerTable,

		...
		if c != nil {
			...
			if modifier := c.Modifier; c.Modifier.IsValid() {
				...
				case models.CriterionModifierMatchesRegex:
					...
					clause := makeClause(fmt.Sprintf("%s IS NOT NULL AND %s IS NOT NULL AND regexp_like(%s, ?)", pathColumn, basenameColumn, filepathColumn), c.Value)
					clause2 := makeClause(fmt.Sprintf("%s IS NOT NULL AND regexp_like(%[1]s, ?)", folderPathColumn), c.Value)
					f.whereClauses = append(f.whereClauses, orClauses(clause, clause2))
				case models.CriterionModifierNotMatchesRegex:
					...
					f.addWhere(fmt.Sprintf("%s IS NULL OR NOT regexp_like(%[1]s, ?)", folderPathColumn), c.Value)

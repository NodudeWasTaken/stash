	...
	return compoundHandler{
		...
		criterionHandlerFunc(func(ctx context.Context, f *filterBuilder) {
			if sceneFilter.Oshash != nil {
				...
				f.addLeftJoin(fingerprintTable, "fingerprints_oshash", "scenes_files.file_id = fingerprints_oshash.file_id AND scenes_files.\"primary\" = true AND fingerprints_oshash.type = 'oshash'")
			}
		...
		criterionHandlerFunc(func(ctx context.Context, f *filterBuilder) {
			if sceneFilter.Checksum != nil {
				qb.addSceneFilesTable(f)
				f.addLeftJoin(fingerprintTable, "fingerprints_md5", "scenes_files.file_id = fingerprints_md5.file_id AND scenes_files.\"primary\" = true AND fingerprints_md5.type = 'md5'")
			}
...
func (qb *sceneFilterHandler) addFilesTable(f *filterBuilder) {
	qb.addSceneFilesTable(f)
	f.addLeftJoin(fileTable, "", "scenes_files.file_id = files.id AND scenes_files.\"primary\" = true")
}
...
func (qb *sceneFilterHandler) addVideoFilesTable(f *filterBuilder) {
	qb.addSceneFilesTable(f)
	f.addLeftJoin(videoFileTable, "", "video_files.file_id = scenes_files.file_id AND scenes_files.\"primary\" = true")
}
...
func (qb *sceneFilterHandler) isMissingCriterionHandler(isMissing *string) criterionHandlerFunc {
	return func(ctx context.Context, f *filterBuilder) {
		if isMissing != nil && *isMissing != "" {
			...
			case "phash":
				qb.addSceneFilesTable(f)
				f.addLeftJoin(fingerprintTable, "fingerprints_phash", "scenes_files.file_id = fingerprints_phash.file_id AND scenes_files.\"primary\" = true AND fingerprints_phash.type = 'phash'")

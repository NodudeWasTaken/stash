...
func (qb *SceneStore) makeQuery(ctx context.Context, sceneFilter *models.SceneFilterType, findFilter *models.FindFilterType) (*queryBuilder, error) {
	...
	if q := findFilter.Q; q != nil && *q != "" {
		query.addJoins(
			...
			join{
				table:    fileTable,
				onClause: "scenes_files.file_id = files.id AND scenes_files.\"primary\" = true",
			},
			join{
				table:    folderTable,
	...
	if options.TotalDuration {
		query.addJoins(
			...
			join{
				...
				onClause: "scenes_files.file_id = video_files.file_id AND scenes_files.\"primary\" = true",
			},
		)
	...
	case "perceptual_similarity":
		...
		query.addJoins(
			join{
				...
				onClause: "scenes_files.file_id = fingerprints_phash.file_id AND scenes_files.\"primary\" = true AND fingerprints_phash.type = 'phash'",
			},
		)
